    ...
    double MapleGenVar1, MapleGenVar2;
      ...
      t24 = 2.0*YY[2];
      t27 = 2.0*YY[0];
      detJ[0][0] = -(t21*((YY[1]-YY[2]-YY[4]+YY[5])*XX[0]+(YY[2]+YY[3]-YY[5]-YY[0])*XX
[1]+(YY[0]-YY[1]-YY[3]+YY[4])*XX[2]+YY[0]*t8+t10*YY[1]+t13+(YY[4]-YY[5])*XX[3]+
(-YY[3]+YY[5])*XX[4]+XX[5]*(-YY[4]+YY[3]))+xi[2]*(XX[0]*(-t23+t24+YY[4]-YY[5])+
XX[1]*(-t24-YY[3]+YY[5]+t27)+XX[2]*(t23+YY[3]-YY[4]-t27)-YY[0]*t8-t10*YY[1]-t13
)+(YY[1]-YY[2])*XX[0]+(YY[2]-YY[0])*XX[1]-XX[2]*(YY[1]-YY[0]))*(Z[0]-Z[1]);
   }
